...
#+++	NOTE: differs from Eddy.R in that here the class sEddyProc2 is initialized
#+++			which allows for daytime resp to be passed
#++ 	T. Keenan modification, November 2018
...
sEddyProc2 <- setRefClass('sEddyProc2', fields=list(
  ...
  )
{
    ...
    fCheckHHTimeSeries(Data.F[,ColPOSIXTime.s], DTS.n=DTS.n, 'sEddyProc2.initialize')
    ...
    fCheckColNum(Data.F, setdiff(ColNames.V.s,ColNamesNonNumeric.V.s), 'sEddyProc2.initialize')
    fCheckColPlausibility(Data.F, ColNames.V.s, 'sEddyProc2.initialize')
